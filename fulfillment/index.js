...
function zn(prov) { return BKK.has(prov)?'BKK_BKK':'BKK_OTHER'; }
...
function interp(rows, t) {
  ...
  if (t <= s[0].k) {
    ...
    const r = (s[1].c-s[0].c)/(s[1].k-s[0].k);
    const pr = (s[1].p-s[0].p)/(s[1].k-s[0].k);
  ...
  if (t >= s[s.length-1].k) {
    ...
    const r = (b.c-a.c)/(b.k-a.k);
    const pr = (b.p-a.p)/(b.k-a.k);
    return {c:Math.round((b.c+r*(t-b.k))*100)/100,p:Math.round((b.p+pr*(t-b.k))*100)/100,e:false};
  }
  for (let i=0;i<s.length-1;i++) {
    const a=s[i],b=s[i+1];
    if (t>=a.k && t<=b.k) {
      if (t===a.k) return {c:a.c,p:a.p,e:true};
      if (t===b.k) return {c:b.c,p:b.p,e:true};
...
function volWt(cr, volCm3) {
  const d = VOL[cr];
  if (!isFinite(d)) return 1;
  if (cr==='DPTHAIPOST') return volCm3;
  return Math.ceil(volCm3/d);
}

function fw(cr, wKg, w,l,h) {
  const v = w*l*h;
  if (cr==='DPTHAIPOST') return Math.max(Math.ceil(wKg*1000),v);
  return Math.max(Math.ceil(wKg), volWt(cr,v));
}
...
function policy(cr, wKg, w,l,h) {
  ...
  let f;
  if (cr==='DPFLASHLIVEBULKY') {f=Math.max(ac,sw(cr,ds));return f>ac?'d':'w';}
  if (cr==='DPFLASHA') {f=Math.max(ac,Math.ceil(v/6000),sw(cr,ds));}
  else {f=Math.max(ac,sw(cr,ds));return f>ac?'d':'w';}
  return f>ac?'d':'w';
}
...
function calcPrice(cr, prov, wKg, wCm, lCm, hCm) {
  ...
  const z = zn(prov);
  const ct = RT[cr];
  if (!ct) return {price:0,rejected:true,reason:'ไม่พบข้อมูลขนส่ง'};
  const zt = ct[z];
  if (!zt) return {price:0,rejected:true,reason:'ไม่มีข้อมูลโซนนี้'};
...
function compareAll(cr, prov, wKg, wCm, lCm, hCm) {
  return Object.keys(RT).map(c => ({courier:c,...calcPrice(c,prov,wKg,wCm,lCm,hCm)})).sort((a,b)=>(a.price||0)-(b.price||0));
}
...
function normProv(raw) {
  ...
  for (const [s,c] of Object.entries(PS)) { if (s.toLowerCase()===inp.toLowerCase()) return c; }
...
// ── Parse Dialogflow parameter value (handles proto struct or plain value) ──
function pv(p) {
  if (p === null || p === undefined) return null;
  if (typeof p === 'object') return p.stringValue || (p.structValue?.fields?.city?.stringValue) || p.numberValue || null;
  return p;
}
...
function ship(params) {
  const f = params||{};
  let raw = pv(f.province);
  const w = parseFloat(pv(f.weight))||0;
  const l = parseFloat(pv(f.length))||0, wd = parseFloat(pv(f.width))||0, ht = parseFloat(pv(f.height))||0;
  if (!raw && !w) return 'กรุณาระบุปลายทางและน้ำหนักครับ เช่น "ส่งของไปเชียงใหม่ 5 กิโล"';
  if (!raw) return 'กรุณาระบุปลายทางจังหวัดครับ';
  if (!w) return 'กรุณาระบุน้ำหนักกี่กิโลกรัมครับ';
  const prov = normProv(raw);
  if (!prov) return `ไม่พบจังหวัด "${raw}" ครับ`;
  const dim = (l&&wd&&ht)?{w:wd,l:l,h:ht}:defDim(w);
  const results = compareAll(null,prov,w,dim.w,dim.l,dim.h);
  const lines = results.map(r => {
    const n = CN[r.courier]||r.courier;
    if (r.rejected) return `- ${n}: ❌ ${r.reason}`;
    return `- ${n}: ${r.price} บาท`;
  });
  const dsp = prov!==raw?`${prov} (${raw})`:prov;
  return `ค่าส่งไป${dsp} ${w} kg\n${lines.join('\n')}`;
}

function checkTime() {
  const {open,dn,ts,r} = isOpen();
  const cs = String(r.c).padStart(2,'0')+':00';
  if (open) return `ขณะนี้ (${dn} ${ts} น.) ร้านเปิดอยู่ค่ะ ปิด ${cs} น.`;
  return `ขณะนี้ (${dn} ${ts} น.) ร้านปิดแล้วค่ะ เปิดอีกที${dn==='เสาร์'||dn==='อาทิตย์'?'วันจันทร์':'พรุ่งนี้'} 09:00 น.`;
}

function tellTime() { return 'ร้านเปิด จันทร์-ศุกร์ 09:00-18:00 น.\nเสาร์-อาทิตย์ 09:00-17:00 น.'; }

// ── Cloud Function entry ───────────────────────────────────────
exports.fulfillment = (req, res) => {
  const intent = (req.body.queryResult||{}).intent||{};
  const params = (req.body.queryResult||{}).parameters||{};
  const name = intent.displayName||'';
  let text;
  if (name==='ส่งของ') text = ship(params);
  else if (name==='เช็คเวลาเปิด') text = checkTime();
  else if (name==='แจ้งเวลาเปิด') text = tellTime();
